...
class InstructionFrame(ctk.CTkFrame):
    ...
    def __init__(self,master):
        ...
        self.master = master
        instruction_str =  """1. Open brawl stars on your device, select the brawler that you wish use and go to the training ground.
    ...
    def goto_hsf_frame(self):
        try:
            wincap = ADBCapture(Constants.adb_device_id, Constants.screen_resolution)
            self.place_forget()
            ...
            self.count +=1
            if self.count <= 1:
                self.message.set("Please connect your device and open Brawl Stars, then try again!")
            else:
                self.message.set("""Please connect your device and open Brawl Stars, then try again!
                                    \nEnsure ADB is running and device is connected.
                                    \nCheck 'adb devices' for your device ID.""")
